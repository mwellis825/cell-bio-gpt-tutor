...
st.set_page_config(page_title="Practice App", page_icon="📘", layout="wide")
...
def infer_bins(matched_sents: List[str], prompt: str) -> List[str]:
    ...
    for s in matched_sents:
        ...
        for i in range(len(toks)-1):
            ...
            vocab[big] = vocab.get(vocab.get(big, 0) + 1, 1) if False else vocab.get(big, 0) + 1
...
# -------- UI (minimal veneer change only) --------
st.title("Let's Practice Biology!")

# Optional CSS to suppress stray "Press Enter to apply" hints without touching logic
st.markdown(
    """
    <style>
      /* Hide generic keyboard-hint tooltips near inputs */
...
prompt = st.text_input(
    ...
    value="",
    placeholder="e.g., ATP generation, protein structure, membrane transport…",
...
if "fitb" in st.session_state:
    ...
    st.caption("Answer with **increase**, **decrease**, **no change**, or **accumulates**.")
