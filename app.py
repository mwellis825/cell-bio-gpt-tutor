...
import os, io, json, base64, zipfile, pathlib, re, uuid
...
def search_chunks(query: str, k: int = 8) -> List[Dict]:
    ...
    try:
        ...
        # fallback to naive if embeddings call failed
        st.warning("Query embedding failed; falling back to keyword retrieval.")
        return search_chunks.__wrapped__(query, k)  # type: ignore
...
# -------------------------------
# Inline H5P renderer (LOCAL assets, injected once)
# -------------------------------
def ensure_h5p_assets_loaded() -> bool:
    """Read local vendor files into session once."""
    try:
...
def render_h5p_inline(h5p_bytes: bytes, comp_id: str, height: int = 560):
    """
    Render a .h5p package inline using locally vendored h5p-standalone assets (no CDN).
    Injects JS/CSS only once; uses a unique container per component via comp_id.
    """
    if not h5p_bytes:
        return

    if not ensure_h5p_assets_loaded():
        # Provide download fallback at least
    ...
    # First render: inject CSS + both JS bundles, then boot.
    if not st.session_state.H5P_LIBS_INJECTED:
        ...
        # Subsequent renders: only boot new container, don’t re-inject libraries.
        html = f"""
        <div id="{container_id}"></div>
        <script>
          (function() {{
    ...
    # Unique, stable key using run_id
    run_id = st.session_state.run_id or "norun"
    st.download_button("⬇️ Download activity (.h5p)",
                       data=h5p_bytes, file_name=f"activity_{comp_id}.h5p",
                       mime="application/zip", key=f"dl-{comp_id}-{run_id}")
...
# Cache structure per run
if generate_clicked:
    # New run id so widget keys remain stable during interactions
    ...
    # Generate once, store in cache, avoid re-gen on interactions
    try:
        ...
        st.session_state.cache[st.session_state.run_id]["fib_bytes"] = fib_bytes
    except Exception as e:
    ...
    if st.session_state.gen_mcq:
        try:
            ...
            st.session_state.cache[st.session_state.run_id]["mcqs"] = mcqs
        except Exception as e:
    ...
    if st.session_state.gen_dnd:
        try:
            ...
            if pairs:
                ...
                st.session_state.cache[st.session_state.run_id]["dnd_bytes"] = dnd_bytes
        except Exception as e:
            st.warning(f"DnD generation failed: {e}")

# Render the last generated run if present
